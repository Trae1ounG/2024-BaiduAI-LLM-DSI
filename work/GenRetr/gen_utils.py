...
def prepare_data_loader(filepath, tokenizer, args, load_type):
    """
      prepare_data_loader
    """
    def read(data_path):
        ...
        with open(data_path, 'r', encoding='utf-8') as f:
            for idx, line in enumerate(f):
                js = json.loads(line)
                yield {'source': js['src'], 'target': js['tgt']}
            print("total line:", idx)

    _ds = load_dataset(read, data_path=filepath, lazy=False)
    _ds, _data_loader = create_data_loader(_ds, tokenizer, args, load_type)
    return _ds, _data_loader
...
def select_sum(ids, scores, tokenizer, max_dec_len=None, num_return_sequences=1):
    ...
    if scores is not None:
        ...
        for preds in group:
            ...
            preds = preds[:][:]
            preds = "\t".join([str(x[0]) for x in preds])
